...
//todo: last before video_trachchange...
function video_trackshow()		{
	if(_video){
		console.log(_video);
		var tracks = getTracks(_video);
		//console.log('video_trackshow : track ');
		//console.log(tracks);
		if(tracks.length){
			var track = tracks[0];
...
function video_trackhide()		{
	if(_video){
		console.log(_video);
		var tracks = getTracks(_video);
		//console.log('video_trackhide : track ');
		//console.log(tracks);
		if(tracks.length){
			var track = tracks[0];
			//console.log('START track HIDE : ');
			if(track.mode ){
				track.mode = 'hidden';
				//console.log('END track HIDE : ');
				return track.mode;
			}
		}
...
function video_trackmode() {
	if(_video){
		console.log(_video);
		var tracks = getTracks(_video);
		console.log('video_trackmode : track ');
		//console.log(tracks);
		if(tracks.length){
			var track = tracks[0];
			console.log('video_trackmode : ');
			if(track){
				if (track.mode != null) {
					//track.mode = 'hidden';
					console.log('video_trackmode : ');
...
function fullexitHandler()
{
	...
	//player_skin.mcFullscreenCancel(true);
        ...
        if (_video_repeatstate == 1) {
			if (posi > _video_repeatright + ADJUST_NUM) {
                ...
                if (getRepeatCnt < repeatedCnt) {
					...
					setCookie('repeat-left', '', -1);
					setCookie('repeat-right', '', -1);
