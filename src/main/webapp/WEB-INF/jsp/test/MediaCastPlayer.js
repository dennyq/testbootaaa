...
//-----------------------------------------------------------------------------
	...
	function set_autosizing(chk) {
		__player_chk_autosizing = chk;
	}

	function set_videosize(x, y) {
		__player_video_width = x;
		__player_video_height = y;
	}

	function set_bookmarkmenu(chk) {
		__player_chk_bookmarkmenu = chk;
	}

	function set_skinmenu(chk) {
		__player_chk_skinmenu = chk;
	}

	function set_fitscreen(chk) {
		__player_fitscreen = chk;
	}

	function keeper_checkterminal(chk) {
		__keeper_terminal = chk;
	}

	function keeper_checkvolume(chk) {
		__keeper_volume = chk;
	}

	function keeper_checkvirtual(chk) {
		__keeper_virtual = chk;
	}

	function keeper_killmessenger(chk) {
		__keeper_messenger = chk;
	}

	function keeper_checkapple(chk) {
		__keeper_apple = chk;
	}
...
//------------------------------------------------------------------
	...
	function set_caption_style() {
		__player_caption_style = cap_transparency + "," + cap_font_name + "," + cap_font_size + "," + cap_font_color + "," + cap_overlay + "," + cap_shadow_color;
	}

	function set_caption_file(f) {
		__player_caption_file = f;
	}

	function set_caption_color(r, g, b) {
		cap_font_color = r + "," + g + "," + b;
		set_caption_style();
	}

	function set_caption_shadow_color(r, g, b) {
		cap_shadow_color = r + "," + g + "," + b;
		set_caption_style();
	}

	function set_caption_fontsize(f) {
		cap_font_size = f;
		set_caption_style();
	}

	function set_caption_font(f) {
		cap_font_name = f;
		set_caption_style();
	}

	function set_caption_overlay(f) {
		cap_overlay = f;
		set_caption_style();
	}
	...
	function set_caption_flash(f) {
		__player_caption_flash = f;
	}
...
//------------------------------------------------------------------
	...
	function set___player_bglogo(url) {
		__player_bglogo = url;
	}

	function set_mirroring(chkEnable) {
		__player_mirroring = chkEnable == "1" ? 1 : 0;
	}

	function set_caching(cachesize) {
		if (cachesize < 1 || cachesize > 1024) {
...
//var __player_keeper_notinstalled =false;
	...
	function startKeeper() {
		...
		if (__player_type == 1) setTimeout(function () {
			keeper_runner_html5(appUri);
		}, 2000);
		if (__player_type == 2) setTimeout(function () {
			keeper_runner_flash(appUri);
		}, 2000);
	}

//function onMaypleCallback(param, value) { alert( 'OnMaypleCallback : param='+param + ',value='+value); }
	function onMaypleCallback(param, value) {
	...
	$(window).resize(function () {
		video_resize();
	}); // player-html5.js 와 player-flash.js 에 정의되어있음
	;//-------------------------------------------
// player-html5.js
...
//재생 상태 확인
	...
	function video_repeatleft(x) {
		_video_repeatleft = x;
	}

	function video_repeatright(x) {
		_video_repeatright = x;
	}

	function video_repeatdo() {
		_video_repeatstate = 1;
	}

	function video_repeatstart() {
		_video_repeatstate = 1;
	}

	function video_repeatend() {
		_video_repeatstate = 0;
	}

	function video_repeatstop() {
		_video_repeatstate = 0;
	}

	function video_fastreverse() {
		video_setposition(video_getposition() - 15);
	}

	function video_fastforward() {
		video_setposition(video_getposition() + 15);
	}
...
//-----------------------
	...
	function set_bookmark_data(x) {
		_video_bookmark = x;
	}

	function set_bookmark_url(x) {
		_video_bookmarkurl = x;
	}

	function set_bookmark_key(site, id, title) {
		_video_bookmarkparam = "site=" + site + "&id=" + id + "&f=" + title;
	}
...
//----------------------------------
	...
	function set_lms_init() {
		...
		try {
			if (__player_lms_progress < 100) set_lms_check_position();
		} catch (e) {
		}
		try {
			if (__player_lms_sposi > 0)  video_setposition(__player_lms_sposi);
		} catch (e) {
		}
	}
